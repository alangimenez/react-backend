...
const { controlId } = require('../middlewares/carrito.mid')

// crear un carrito
router.post('/', crearCarrito) //ok

// lee el archivo de carrito, revisa si el carrito existe, y lo elimina.
router.delete('/:id', controlId, eliminarCarrito) //ok
...
router.post('/:idCarr/productos/:idProd', validarCarrito, prodAlCarrito) //ok

// lista los productos de un carrito, y devolvera error si el carrito no existe.
router.get('/:idCarr/productos', controlId, prodDelCarrito) //ok
...
router.delete('/:idCarr/productos/:idProd', validarDelete, elimProdDelCarrito) //ok
